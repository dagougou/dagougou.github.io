(function () {    
    window.Sheet = function () {
        this.id = [3778678, 3779629, 4395559, 64016, 112504, 19723756, 2884035, 2250011882, 1978921795, 11641012];
        //491416585
        this.userId = null;
        this.sheetList = [];
    ...
    Sheet.prototype.useHelp = function () {
        var isOpen = confirm(
            '⚪ 设置用户\n' +
            '1、首先打开网易云音乐官网(点击 确定 自动打开)\n' +
            '2、然后点击页面右上角的“登录”，登录您的账号\n' +
            '3、点击您的头像，进入个人中心\n' +
            '4、此时浏览器地址栏 /user/home?id= 后面的数字就是您的网易云 ID\n\n' +
            '⚪ 添加歌单\n' +
            '1、同样点击 确定 打开网易云音乐官网\n' +
            '2、找到一个需要添加的歌单并点击进入\n' +
            '3、此时浏览器地址栏playlist?id= 后面的数字就是歌单的ID'
        );
        if (isOpen) {
            open('https://music.163.com/');
        }
    }
